...
utl::report "DYNAMIC PDN CONFIG CHECK: Using vertical net pitch step -> $::env(PDN_VPITCH) um"
# =============================================================================

define_pdn_grid -name stdcell_grid -starts_with GROUND -voltage_domains CORE

# 1. Vertical Metal3 power mesh snapping perfectly onto your tracks using the direct 11.04 um cycle step
add_pdn_stripe -grid stdcell_grid -layer $::env(PDN_VERTICAL_LAYER) -width $::env(PDN_VWIDTH) -pitch $::env(PDN_VPITCH) -offset $actual_core_left -spacing $::env(PDN_VSPACING) -nets "$::env(VDD_NET) $::env(GND_NET)" -extend_to_boundary
...
# 3. Connect the rails directly to the vertical mesh stripes (with intermediate layer mapping)
